#!/usr/bin/env python3
"""
Analyze bias via slicing on LLM synthetic eval outputs.

Input: a CSV like results.csv with columns:
- city_tier, budget_tier, distance_bucket, car_ratio_bucket, dietary_flag
- json_valid, options_count_ok, budget_compliance, distance_compliance, logistics_feasible

Output:
- prints overall + slice tables
- writes a Markdown report

Usage:
  python analyze_bias_slices.py --csv results.csv --out bias_slicing_report.md
"""
import argparse
...
- Slice: **{worst["city_tier"]} × {worst["budget_tier"]}** (n={int(worst["n"])})
...
This baseline run suggests a **coverage/constraint disparity**: some slices (typically low-budget + sparse venue availability) violate budget constraints more often.

Planned mitigations (system-level, since we are not fine-tuning):
1. **Prefilter** candidate venues by budget before generation.
2. **Validate → repair**: if any option violates budget, re-prompt once to correct.
3. **Fallback mode** when too few compliant venues exist (free/low-cost activity templates).

Trade-offs:
- repair increases cost/latency (extra call on some requests),
- prefilter may reduce diversity in low-coverage slices.
"""
