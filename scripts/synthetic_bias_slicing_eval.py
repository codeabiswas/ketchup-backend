...
def parse_json_safe(text: str) -> Optional[Dict[str, Any]]:
    try:
        ...
        # attempt recover if model wraps JSON in extra text
        start = text.find("{")
        end = text.rfind("}")
...
def call_vllm_chat(
    ...
    """
    Returns (assistant_text, use_schema_after_call).
    If schema is rejected by server/model, automatically retries once without schema.
    """
    ...
    # If schema causes a 400, retry once without schema
...
def score(sample: Dict[str, Any], out: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # logistics feasibility heuristic
    ...
    if car_ratio == 0.0 and is_far:
        # should mention transit/meetpoint/rideshare rather than "assign driver"
        if any(
            k in plan
            for k in [
                "transit",
